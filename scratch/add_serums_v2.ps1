$path = "d:\wamp64\www\monster\src\data\resources.ts"
...
$insertion = '  },
  ...
  "xp_serum_3": {
    ...
    "category": "relic",
    "rarity": "Legendární",
    "description": { "cz": "Maximální genetický stimulant, který okamžitě přidá 700 XP.", "en": "Ultimate genetic stimulant that instantly adds 700 XP.", "sk": "Maximálny genetický stimulant, který okamžitě přidá 700 XP." },
    "dropWeight": 1, "dropMin": 1, "dropMax": 1,
    "stats": { "xp": 700 },
    "hasCustomIcon": true, "customIcon": "xp_serum_3"
  }
};'
# Escape regex characters if needed, but here we want exact match for };
# Since }; is at the end, we can replace it.
if ($content.EndsWith("};`r`n") -or $content.EndsWith("};`n") -or $content.EndsWith("};")) {
    $newContent = $content.Substring(0, $content.LastIndexOf("};")) + $insertion
    Set-Content $path $newContent -NoNewline -Encoding utf8
} else {
    Write-Error "Could not find }; at the end of the file"
}
