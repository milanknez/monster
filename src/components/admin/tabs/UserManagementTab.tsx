...
import { 
  ...
  MapPin, Clock, Sword, Heart, AlertCircle, Mail, LogIn
} from 'lucide-react';
...
export const UserManagementTab: React.FC<UserManagementTabProps> = ({ players, selectedPlayerId, setSelectedPlayerId }) => {
    ...
    for (const [ruid, data] of entries) {
      ...
      if (targetUid && !targetUid.includes('@')) {
        ...
        if (p) {
          ...
          if (data.level !== currentLevel || data.totalXP !== (p.totalXP || 0) || !data.registeredUid || data.status !== 'registered') {
  ...
  const currentName = detailedData?.name || detailedData?.playerName || pSummary?.name || selectedPlayerId;
  ...
  const mapContainerRef = useCallback((node: HTMLDivElement | null) => {
    if (node !== null) {
      ...
      const map = L.map(node, {
          ...
          zoom: 16,
          zoomControl: true, // Zapnuto pro kontrolu
  ...
  const handleManualLink = async () => {
    if (!manualEmail || !manualUid) return alert('Vyplňte obě pole!');
      ...
      <div className="mb-6 space-y-4">
        <div className="flex items-center justify-between bg-slate-900 border border-white/5 p-4 rounded-3xl">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Administrace</div>
                  <div className="text-sm font-black text-white uppercase italic">Správa komunity</div>
               </div>
            </div>
            <button 
            ...
            >
              ...
              {isRepairing ? 'Automatická oprava' : 'Automaticky propojit vše'}
            </button>
        </div>
          ...
          <div className="space-y-6 pb-20">
              ...
              <div className="p-8 bg-slate-900 border border-white/10 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                <div className="flex items-center gap-6 relative z-10">
                  <div className="size-20 rounded-2xl bg-slate-800 border-2 border-white/10 p-1 flex items-center justify-center">
                    <img src={`https://api.dicebear.com/7.x/${detailedData?.avatarStyle || pSummary?.avatarStyle || 'bottts'}/svg?seed=${detailedData?.avatarSeed || pSummary?.avatarSeed || selectedPlayerId}`} className="w-full h-full rounded-lg" />
                  </div>
                ...
                <div className="mt-6 p-4 bg-black/40 rounded-2xl border border-white/5 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">Poslední přihlášení</div>
                    <div className="text-xs font-black text-white italic flex items-center gap-2">
                       <LogIn size={14} className="text-primary" />
                       {new Date(pSummary?.lastActive || Date.now()).toLocaleString('cs-CZ')}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">Aktivita</div>
                    <div className="text-[10px] font-black text-emerald-500">
                      {pSummary?.isOnline ? 'PRÁVĚ ONLINE' : 'OFFLINE'}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3 mt-6 text-center">
                    <div className="bg-black/40 p-3 rounded-2xl">
                      <div className="text-[9px] text-slate-500 font-bold uppercase">Level</div>
                      <div className="text-xl font-black text-white italic">
                        {detailedData?.level || detailedData?.currentLevel || pSummary?.level || 1}
                      </div>
                    </div>
                    <div className="bg-black/40 p-3 rounded-2xl"><div className="text-[9px] text-slate-500 font-bold uppercase">Předměty</div><div className="text-xl font-black text-emerald-500 italic">{detailedData?.inventory?.length || 0}</div></div>
                    <div className="bg-black/40 p-3 rounded-2xl"><div className="text-[9px] text-slate-500 font-bold uppercase">Monstra</div><div className="text-xl font-black text-primary italic">{detailedData?.caughtMonsters?.length || pSummary?.monsterCount || 0}</div></div>
                </div>
              </div>
              ...
              <div className="h-[320px] bg-slate-900 border-2 border-white/10 rounded-[2.5rem] overflow-hidden relative shadow-2xl">
                <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-1 pointer-events-none">
                    <div className="bg-slate-950/90 px-3 py-1.5 rounded-xl border border-white/10 text-[10px] font-black text-white uppercase flex items-center gap-2 shadow-2xl backdrop-blur-md">
                        <MapPin size={12} className="text-cyan-400" /> Detekce Polohy
                    </div>
                    {hasCoords && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h2 className="text-sm font-black text-slate-500 uppercase tracking-widest px-2 flex items-center gap-2"><Sword size={16} className="text-primary" /> Sbírka monster</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {detailedData?.caughtMonsters?.map((m: any, i: number) => (
                    <div key={i} className="p-3 bg-slate-900/50 border border-white/5 rounded-2xl flex items-center gap-3">
                      <div className="size-10 bg-black/40 rounded-lg p-1 shrink-0"><img src={`/monsters/${m.id}.png`} className="w-full h-full object-contain" /></div>
                      <div className="min-w-0">
                        <div className="text-[11px] font-black text-white uppercase truncate">{m.name}</div>
                        <div className="text-[9px] font-black text-primary">Level {m.level}</div>
                      </div>
                    </div>
                  )) || <div className="py-10 text-center opacity-20 italic font-black uppercase text-[10px] tracking-widest border border-white/5 rounded-3xl">Prázdná sbírka</div>}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {detailedData?.referralList && Object.entries(detailedData.referralList).length > 0 ? (
                    Object.entries(detailedData.referralList).map(([ruid, data]: [string, any]) => {
                      ...
                      const targetUid = data.registeredUid;
                          ...
                          isReady ? "border-purple-500/40 bg-purple-500/5 shadow-lg shadow-purple-500/10" : "border-white/5"
                           ...
                           <div className="flex-1 min-w-0 relative z-10">
                              <div className="text-xs font-black text-white truncate uppercase italic">
                                {data.name || ruid?.split('@')[0] || 'Neznámý'}
                              </div>
                    ...
                    <div className="py-12 text-center bg-slate-900/50 border border-white/5 rounded-[2rem] border-dashed">
                       ...
                       <div className="text-[10px] opacity-20 italic font-black uppercase tracking-widest">Žádné pozvánky</div>
                    </div>
                  )}
                </div>
              </div>
