...
import { 
  ...
  Sparkles, Info, Activity, ChevronRight, Star, Target, Gem
} from 'lucide-react';
    ...
    <div className="absolute inset-x-0 inset-y-0 pointer-events-none z-30">
      ...
      {isEpic && (
        <motion.div
          ...
          className="absolute inset-4 rounded-[3.5rem] bg-purple-500/5 blur-2xl pointer-events-none"
        />
      )}

      {/* Heavy Beveled Outer Frame */}
      <div className={cn(
        "absolute inset-2 border-[6px] rounded-[3.5rem] border-double shadow-[0_0_40px_rgba(0,0,0,0.8)]",
    ...
    <div className="relative group min-w-[64px]">
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1 mb-0.5">
          ...
          <span className={cn("text-[8px] font-black uppercase tracking-[0.2em] leading-none whitespace-nowrap", iconColor)}>Score</span>
        </div>
      ...
      >
        ...
        <div className="px-5 space-y-8 mt-6 relative z-10">
          ...
          <div className="relative group">
            {/* Visual Backdrops */}
            <div className={cn(
              "absolute inset-0 rounded-[3rem] blur-3xl opacity-20 transition-opacity group-hover:opacity-30",
            ...
            <div className="relative aspect-square w-full bg-slate-800 rounded-[3rem] border-4 border-white/10 overflow-hidden shadow-2xl flex items-center justify-center group-hover:border-white/20 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-4">
                          ...
                          subValue={
                            <div className="flex flex-col gap-1.5 mt-1">
                              <div className="text-[8px] font-black text-emerald-500/80 bg-emerald-500/5 px-2 py-0.5 rounded-full border border-emerald-500/10 w-fit">
                                Bonus {maxHP - (monster.stats?.hp || 100)}
                              </div>
                              {isDamaged && (
             ...
             <div className="bg-white/[0.07] border border-white/10 rounded-[2rem] p-4 flex flex-col items-center justify-center gap-2 backdrop-blur-md shadow-2xl transition-all relative overflow-hidden group hover:bg-white/[0.1] hover:border-white/20">
                ...
                <div className="text-[8px] font-black text-emerald-500/80 bg-emerald-500/5 px-2 py-0.5 rounded-full border border-emerald-500/10">
                   Bonus {getMonsterAttack(monster) - (monster.stats?.attack || 10)}
                </div>
             </div>
             ...
             <div className="bg-white/[0.07] border border-white/10 rounded-[2rem] p-4 flex flex-col items-center justify-center gap-2 backdrop-blur-md shadow-2xl transition-all relative overflow-hidden group hover:bg-white/[0.1] hover:border-white/20">
                ...
                <div className="text-[8px] font-black text-emerald-500/80 bg-emerald-500/5 px-2 py-0.5 rounded-full border border-emerald-500/10">
                   Bonus {getMonsterDefense(monster) - (monster.stats?.defense || 10)}
                </div>
             </div>
          </div>
          ...
          {/* --- Description & Bio --- */}
          <div className="bg-white/[0.03] border border-white/5 rounded-[2.5rem] p-6 shadow-inner relative overflow-hidden">
          ...
          {/* --- Equipment Section --- */}
          <div className="bg-[#0f141d]/80 backdrop-blur-md rounded-[2.5rem] border border-white/10 p-6 shadow-2xl relative">
        ...
        <div className="px-6 mt-16 mb-12">
          <motion.button
            ...
            className={cn(
              "w-full group relative py-6 rounded-[2.5rem] overflow-hidden transition-all border-2 shadow-2xl",
              canRelease
        ...
        <AnimatePresence>
          {showHealingModal && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-end flex-col">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowHealingModal(false)} className="absolute inset-0 bg-black/80 backdrop-blur-xl" />
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="w-full max-w-lg bg-[#0d1117] border-t-4 border-emerald-500 rounded-t-[3.5rem] p-8 pb-12 shadow-[0_-20px_80px_rgba(16,185,129,0.3)] relative z-10">
                ...
                <div className="grid grid-cols-1 gap-4 max-h-[45vh] overflow-y-auto pr-2 custom-scrollbar">
                  {inventory?.filter(i => i?.type === 'hp_potion' && i?.count > 0).map(item => (
                    <motion.button key={item?.type} whileTap={{ scale: 0.97 }} onClick={() => { item?.type && onUsePotion?.(item.type); setShowHealingModal(false); }} className="group relative flex items-center gap-5 p-6 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-[2.5rem] hover:border-emerald-500/30 transition-all text-left overflow-hidden">
        ...
        <AnimatePresence>
          {confirmRelease && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setConfirmRelease(false)} className="absolute inset-0 bg-black/90 backdrop-blur-2xl" />
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="w-full max-w-sm bg-[#161a22] border-2 border-red-500/20 rounded-[3.5rem] p-10 text-center shadow-[0_0_120px_rgba(239,68,68,0.25)] relative z-10">
